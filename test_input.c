...
void init(Commands *c){
    ...
    c = append_command_list(c, ID_NEWCOM, "newcom", "", "creates a new command with the response text of your first argument, the help text of the second, and a defined id");
    ...
    c = append_command_list(c, ID_DELCOM, "delcom", "", "deletes a command with a specified ID");
    c = append_command_list(c, ID_DELCOM, "delcom", "", "[b]eletes a command with a specified ID");
    c = append_command_list(c, ID_DELCOM, "delcom", "blah", "blah blah");
}
	...
	while(1){
		...
		if(id == ID_BARRELROLL){
			...
			if(arguments && arguments->next && arguments->next->next && arguments->next->next->next){
                id_count++;
				command_list = append_command_list(command_list, id_count,arguments->next->arg, arguments->next->next->arg, arguments->next->next->next->arg);
